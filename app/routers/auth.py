...
async def login_action(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    # Buscar senha atual armazenada no banco
    stmt = select(Configuracao).limit(1)
    res = await db.execute(stmt)
    config = res.scalars().first()

    # Define a senha correta: Banco > Variável de Ambiente > Padrão
    senha_admin_correta = (
        config.admin_senha
        if (config and config.admin_senha)
